...
public class TrackerWindow {
    ...
    private final TextInputTextView mRootClassNameView;
    ...
    public TrackerWindow(@NonNull Context context) {
        ...
        mRootClassNameView = mView.findViewById(R.id.root);
        ...
        mRootClassNameView.setOnLongClickListener(v -> {
            Editable hierarchy = mRootClassNameView.getText();
    ...
    public void showOrUpdate(AccessibilityEvent event) {
        ...
        if (!mPaused) {
            ...
            mRootClassNameView.setText(TextUtils.join("\n", getClassHierarchies(event)));
        }
    }
    ...
    @NonNull
    private static List<CharSequence> getClassHierarchies(AccessibilityEvent event) {
